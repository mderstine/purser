...
def test_format_binary_status() -> None:
    text = format_binary_status(
        BinaryStatus(name="bd", path="/tmp/bd", version="1.0.0", ok=True)
    )
    assert "bd: ok" in text
    assert "version=1.0.0" in text


def test_ensure_local_beads_context_accepts_embedded_repo_local(
    ...
    def fake_run(*args, **kwargs):
        ...
        return subprocess.CompletedProcess(
            ["bd"], 0, stdout=__import__("json").dumps(payload), stderr=""
        )
...
def test_ensure_local_beads_context_rejects_server_mode(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    payload = {
        "beads_dir": str(tmp_path / ".beads"),
